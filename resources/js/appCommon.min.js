...
if(e){var l=b.find(c);b.find("#header #left").show().touch(function(){b.find("#header #left").hide().off();if(useAnimations)l.changePage(b.find("#main"),true,function(){l.css("visibility","hidden")});else{l.hide();b.find("#main").show().css("visibility","")}b.find("#header #title").text("Settings");ManageUserSession.isActive()&&b.find("#header #done").show();a("#main")})}},m=function(a,b,c){l(a,b,true,c);LocalyticsManager.tagScreen(b)},p=function(){b.positionCenterOf(window)};return{show:function(){var d,
e;if(!c){c=true;b=$j("#app_settings");b.find("#main #connection #hostType").click(function(){m("#hosts","Login Host")});b.find("#hosts table td").click(function(){var a=$j(this);a.addClass("cellselected");ManageUserSession.setLoginHostType(this.id);j();setTimeout(function(){a.removeClass("cellselected")},100)});b.find("#main #connection #host_url input").focus(function(){g=true;$j(this).css("text-align","left")}).blur(function(){g=false;$j(this).val(ManageUserSession.getLoginHostUrl());$j(this).css("text-align",
"right");setTimeout(function(){$j(document.body).scrollTop(0)},10)});b.find("#main #connection #host_url form").submit(function(){var a=b.find("#main #connection #host_url input"),c=b.find("#main #connection #host_url input").val().trim();(c=c.length==0||c.endsWith(".salesforce.com"))||alert('Custom host url must end with ".salesforce.com".');if(c){ManageUserSession.setLoginHostUrl(a.val());j();a.blur()}return false});b.find("#main #help #help_about").click(function(){m("#about","About")});b.find("#main #help #help_faq").click(function(){m("#faq",
"FAQ",function(){})});b.find("#main #help #help_feedback").click(function(){location.href="mailto:"+feedbackEmail});b.find("#main #help #help_eula").click(function(){m("#eula","Contact Viewer EULA",k)});b.find("#loginbtn").off().click(function(){prepareSession()});b.find("#logoutbtn").off().click(function(){confirm("Logout user "+ManageUserSession.getUsername()+"?")&&logout(false)})}j();b.find("#header #title").text("Settings");b.find(".page").css("visibility","hidden");b.find("#main").css("visibility",
...
getBaseUrl()});ManageUserSession.isActive()||b.find("#header #done").hide();d=window.innerHeight;e=(window.innerHeight-b.height())/2;b.hide().css("zIndex",$j.topZIndex(f.elem)+10).css({left:(window.innerWidth-b.width())/2});useAnimations?b.css({top:0}).show().slideIn("Y",d,e):b.css({top:e}).show();a("#main");b.orientationChange(p);LocalyticsManager.tagScreen("Settings")},hide:function(a){var c=window.innerHeight,d=function(){b.hide();f.hide();typeof a=="function"&&a()};useAnimations?b.slideOut("Y",
c,d):d();h();b.unbindOrientationChange(p)},showEula:function(a,c){SettingsManager.show();l("#eula","End User License Agreement",false,function(){k(true);var d=b.find("#eula_buttons").show();d.find("#accept").off().enableTap().click(a);d.find("#decline").off().enableTap().click(c)});LocalyticsManager.tagScreen("EULA Request")},hideEula:function(){SettingsManager.hide(function(){if(useAnimations)b.find("#eula").changePage(b.find("#main"),true);else{b.find("#eula").hide();b.find("#main").show().css("visibility",
...
a)},setLoginHostUrl:function(a){a=a.toLowerCase();if(b()!=a){var d="host_custom";switch(a){case "login.salesforce.com":d="host_production";break;case "test.salesforce.com":d="host_sandbox"}StorageManager.setLocalValue(c,a);ManageUserSession.setLoginHostType(d)}},initialize:function(b,c){e||(e=new sforce.Client);var g=StorageManager.getSessionValue(d),h=StorageManager.getLocalValue("SFDC-OAUTH-TOKEN");if(f)b();else if(g&&g.length>0){e.setSessionHeader(g);var n=function(){e.setSessionHeader(null);StorageManager.clearSessionValue(d);
ManageUserSession.initialize(b)};e.prepareSession(function(c){if(c.success){a(c);typeof b=="function"&&b()}else n()},n)}else if(h&&h.length>0)PasscodeManager.checkPasscode(function(c,d){if(c==-1)confirm("Logout user?")?logout(true):typeof d=="function"&&d(false);else{var f=$j(document).showActivityInd("Authenticating...",false),g=StorageManager.getLocalValue("SFDC-CLIENT-ID"),h=StorageManager.getLocalValue("SFDC-OAUTH-TOKEN");e.refreshAccessToken(c,g,h,function(c){if(c.success){a(c);typeof d=="function"&&
d(true);typeof b=="function"&&b()}else{c.errorCode=="INVALID_TOKEN"&&StorageManager.clearAll();typeof d=="function"&&d(c.success,c.error)}},function(a,b){typeof d=="function"&&d(false,b=="error"?"Server Unavailable. Check network connection or try again later.":"Request timed out. Please try again.")},f.hide)}});else if(window.location.search.indexOf("code=")>0){var q=/code=([^&]*)&?/.exec(window.location.search)[1],o=function(){history.pushState(null,null,window.location.pathname);b()},g=function(b,
c){var b=b==-1?void 0:b,d=$j(document).showActivityInd(b?"Setting Passcode...":"Authenticating...",false);e.obtainAccessToken(ManageUserSession.getLoginHostUrl(),q,b,function(e){d.hide();if(e.success){a(e);var f=e.clientId,e=b?e.refreshToken:null;f&&f.length>0&&StorageManager.setLocalValue("SFDC-CLIENT-ID",f);e&&e.length>0&&StorageManager.setLocalValue("SFDC-OAUTH-TOKEN",e);typeof o=="function"&&o()}else alert(e.error);typeof c=="function"&&c()},function(){if(confirm("Failed to obtain access. Try again!"))window.location=
