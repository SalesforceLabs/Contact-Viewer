...
"flip";h.querySelector(".pullDownLabel").innerHTML="Release to refresh";this.minScrollY=0}else if(this.y<5&&h.className.match("flip")){h.className="";h.querySelector(".pullDownLabel").innerHTML="Pull down to refresh";this.minScrollY=-d}};g=function(){if(h.className.match("flip")){h.className="loading";h.querySelector(".pullDownLabel").innerHTML="Loading...";b()}}}var l={useTransition:!0,topOffset:d||0};c&&c.onBeforeScrollStart&&(l.onBeforeScrollStart=c.onBeforeScrollStart);e&&(l.onRefresh=e);f&&(l.onScrollMove=
f);g&&(l.onScrollEnd=g);return new iScroll(a[0],l)}
...
"/services/apexrest/oauth2/?doAuthorize&host="+a},sforce.Client.prototype.refreshAccessToken=function(a,b,c,d,e,f){this.ajax("POST",getBaseUrl()+"/services/apexrest/oauth2/refreshAccess","rt="+c+"&pass="+a+"&cid="+b,d,e,f)},sforce.Client.prototype.obtainAccessToken=function(a,b,c,d,e,f){this.ajax("POST",getBaseUrl()+"/services/apexrest/oauth2/authenticate","code="+b+(c?"&pass="+c:"")+(a?"&host="+a:""),d,e,f)},sforce.Client.prototype.prepareSession=function(a,b,c){this.ajax("POST",getBaseUrl()+"/services/apexrest/oauth2/prepareSession",
null,a,b,c)},sforce.Client.prototype.prepareSessionFromOAuth=function(a,b,c,d,e,f){this.ajax("POST",getBaseUrl()+"/services/apexrest/oauth2/prepareSession","accessToken="+a+"&instanceUrl="+b+"&identityUrl="+c,d,e,f)},sforce.Client.prototype.revokeSession=function(a,b,c,d,e){this.ajax("POST",getBaseUrl()+"/services/apexrest/oauth2/revokeSession",a&&b?"cid="+a+"&rt="+b:"",c,d,e)},sforce.Client.prototype.queryContactsViaApex=function(a,b,c,d){this.ajax("GET",getBaseUrl()+"/services/apexrest/cvapi","action=fetchContacts&filter="+
a,b,c,d)},sforce.Client.prototype.retrieveContactViaApex=function(a,b,c,d,e){this.ajax("GET",getBaseUrl()+"/services/apexrest/cvapi","action=retrieveContact&id="+a+"&fields="+b,c,d,e)},sforce.Client.prototype.searchContactsViaApex=function(a,b,c,d){this.ajax("GET",getBaseUrl()+"/services/apexrest/cvapi","action=searchContacts&text="+a,b,c,d)},sforce.Client.prototype.fetchChatterViaApex=function(a,b,c,d){this.ajax("GET",getBaseUrl()+"/services/apexrest/cvapi","action=fetchChatter&ids="+a,b,c,d)},sforce.Client.prototype.fetchActivitiesViaApex=
function(a,b,c,d){this.ajax("GET",getBaseUrl()+"/services/apexrest/cvapi","action=fetchActivities&ids="+a,b,c,d)},sforce.Client.prototype.getUsersInfoViaApex=function(a,b,c,d,e){this.ajax("GET",getBaseUrl()+"/services/apexrest/cvapi","action=getUsersInfo&id="+a+"&fetchPhotos="+b,c,d,e)},sforce.Client.prototype.describeContactViaApex=function(a,b,c){this.ajax("GET",getBaseUrl()+"/services/apexrest/cvapi","action=describeContact",a,b,c)},sforce.Client.prototype.getContactDetailsViaApex=function(a,b,
c,d,e){var f=this,g=getBaseUrl()+"/ContactDetails",h=(new Date).getTimezoneOffset();$j.ajax({type:"GET",url:g,data:"id="+a+"&rtid="+(b||"")+"&tzOffset="+h,success:c,error:d,complete:e,beforeSend:function(a){f.sessionHeader&&a.setRequestHeader(f.SESSION_HEADER,f.sessionHeader)}})},sforce.Client.prototype.getContactAppEula=function(a,b,c){var d=getBaseUrl()+"/ContactsAppEula";$j.ajax({type:"GET",url:d,success:a,error:b,complete:c})},sforce.Client.prototype.submitEulaResponse=function(a,b,c,d,e){this.ajax("GET",
getBaseUrl()+"/services/apexrest/cvapi","action=manageEula&clientId="+a+"&eulaResponse="+b,c,d,e)});
var SettingsManager=function(){function a(c){if(c&&e!=c){d&&h();d=createScroller(b.find(".settings_body "+c),null,{onBeforeScrollStart:function(){}});e=c;$j(window).orientationChange(function(){a()})}else d&&d.refresh();b.find(".settings_body "+e+" a").unbind().touch(function(a){a.stopPropagation()})}var b,c,d,e,f,g=false,h=function(){if(d){d.destroy();e=d=void 0}},l=function(){var a=ManageUserSession.getLoginHostType();if(!a){a=b.find("#hosts table td")[0].id;ManageUserSession.setLoginHostType(a)}if(a){b.find("#connection #hostType #name").text(b.find("#hosts #"+
a+" span").text());b.find("#hosts table td").css("color","black");b.find("#hosts #"+a).css("color","#395483").append(b.find("#hosts #check"))}b.find("#main #connection #host_url input").val(ManageUserSession.getLoginHostUrl())},j=function(c){b.find("#header #left").show().touch(function(){b.find("#header #left").hide().unbind();c.changePage(b.find("#main"),true,function(){c.css("visibility","hidden")});b.find("#header #title").text("Settings");ManageUserSession.isActive()&&b.find("#header #done").show();
a("#main")})},m=function(c,d){ManageUserSession.getApiClient().getContactAppEula(function(a){a=$j(a);a.find("head, script").remove();b.find("#eula .content").empty().append(a);if(c){a=b.find("#eula_buttons").show();b.find("#eula").css("margin-bottom",a.outerHeight()+"px")}else b.find("#eula").css("margin-bottom","0")},errorCallback,function(b,c){a("#eula");typeof d=="function"&&d(c)})},o=function(c,d,e,f){g&&setTimeout(function(){b.find("#main #connection #host_url input").blur()},10);var n=$j(this);
n.addClass("cellselected");b.find("#header #done").hide();b.find("#main").changePage(b.find(c),false,function(){n.removeClass("cellselected");b.find("#main").css("visibility","hidden");typeof f=="function"&&f()});b.find("#header #title").text(d);a(c);e&&j(b.find(c))},k=function(a,b,c){o(a,b,true,c)},p=function(){b.find("#main #connection #hostType").click(function(){k("#hosts","Login Host")});b.find("#hosts table td").touch(function(){var a=$j(this);a.addClass("cellselected");ManageUserSession.setLoginHostType(this.id);
l();setTimeout(function(){a.removeClass("cellselected")},100)});b.find("#main #connection #host_url input").focus(function(){g=true;$j(this).css("text-align","left")}).blur(function(){g=false;$j(this).val(ManageUserSession.getLoginHostUrl());$j(this).css("text-align","right");setTimeout(function(){$j(document.body).scrollTop(0)},10)});b.find("#main #connection #host_url form").submit(function(){var a=b.find("#main #connection #host_url input"),c=b.find("#main #connection #host_url input").val().trim();
(c=c.length==0||c.endsWith(".salesforce.com"))||alert('Custom host url must end with ".salesforce.com".');if(c){ManageUserSession.setLoginHostUrl(a.val());l();a.blur()}return false});b.find("#main #help #help_about").click(function(){k("#about","About")});b.find("#main #help #help_faq").click(function(){k("#faq","FAQ",function(){})});b.find("#main #help #help_eula").click(function(){k("#eula","Contact Viewer EULA",m)});b.find("#loginbtn").unbind().click(function(){prepareSession()}).touch(function(a){a.stopPropagation()});
b.find("#logoutbtn").unbind().click(function(){confirm("Logout user "+ManageUserSession.getUsername()+"?")&&logout(false)}).touch(function(a){a.stopPropagation()})},n=function(){b.positionCenterOf(window)};return{show:function(){if(!c){c=true;b=$j("#app_settings");p()}l();b.find("#header #title").text("Settings");b.find(".page").css("visibility","hidden");b.find("#main").css("visibility","visible");b.width()<500&&b.find("#header #left>div").text("Back");if(ManageUserSession.isActive()){b.find("#header #done").show();
b.find("#loginbtn").hide();b.find("#logoutbtn").show()}else{b.find("#loginbtn").show();b.find("#logoutbtn").hide()}f=$j("#loggedin").addOverlay();b.find("#username").setText(ManageUserSession.getUsername()||"None");var d={top:0,left:(window.innerWidth-b.width())/2};b.hide().css(d).css("zIndex",$j.topZIndex(f.elem)+10);var d=window.innerHeight,e=(window.innerHeight-b.height())/2;b.find("#header #done").unbind("click").click(function(){SettingsManager.hide();$j(this).unbind("click");if(!ManageUserSession.isActive())window.location=
getBaseUrl()});ManageUserSession.isActive()||b.find("#header #done").hide();b.show().slideIn("Y",d,e);a("#main");b.orientationChange(n)},hide:function(a){b.slideOut("Y",window.innerHeight,function(){b.hide();f.hide();typeof a=="function"&&a()});h();b.unbindOrientationChange(n)},showEula:function(a,c){SettingsManager.show();o("#eula","End User License Agreement",false,function(){m(true);var d=b.find("#eula_buttons").show();d.find("#accept").unbind().click(a);d.find("#decline").unbind().click(c)})},
...
var ManageUserSession=function(){function a(a){m=true;o=a.username;k=a.eula;j.setSessionHeader(a.sessionToken);StorageManager.setSessionValue(l,a.sessionToken)}function b(b,c){j.setSessionHeader(b);var d=function(){j.setSessionHeader(null);StorageManager.clearSessionValue(l);ManageUserSession.initialize(c)};j.prepareSession(function(b){if(b.success){a(b);typeof c=="function"&&c()}else d()},d)}function c(b){PasscodeManager.checkPasscode(function(c,d){if(c==-1)confirm("Logout user?")?logout(true):typeof d==
"function"&&d(false);else{var e=$j(document).showActivityInd(loadingImg,"Authenticating...",false),k=StorageManager.getLocalValue(g),h=StorageManager.getLocalValue(f);j.refreshAccessToken(c,k,h,function(c){if(c.success){a(c);typeof d=="function"&&d(true);typeof b=="function"&&b()}else{c.errorCode=="INVALID_TOKEN"&&StorageManager.clearAll();typeof d=="function"&&d(c.success,c.error)}},function(a,b){typeof d=="function"&&d(false,b=="error"?"Server Unavailable. Check network connection or try again later.":
"Request timed out. Please try again.")},e.hide)}})}function d(){var a;switch(ManageUserSession.getLoginHostType()){case "host_production":a="login.salesforce.com";break;case "host_sandbox":a="test.salesforce.com";break;case "host_custom":a=StorageManager.getLocalValue(h)}if(!a||a.length==0)a="login.salesforce.com";return a}function e(b,c){var d=function(d,e){var d=d==-1?void 0:d,k=$j(document).showActivityInd(loadingImg,d?"Setting Passcode...":"Authenticating...",false);j.obtainAccessToken(ManageUserSession.getLoginHostUrl(),
b,d,function(b){k.hide();if(b.success){a(b);var n=b.clientId,b=d?b.refreshToken:null;n&&n.length>0&&StorageManager.setLocalValue(g,n);b&&b.length>0&&StorageManager.setLocalValue(f,b);typeof c=="function"&&c()}else alert(b.error);typeof e=="function"&&e()},function(){if(confirm("Failed to obtain access. Try again!"))window.location=j.getAuthorizeUrl(ManageUserSession.getLoginHostUrl())})};p?PasscodeManager.setupPasscode(d):d()}var f="SFDC-OAUTH-TOKEN",g="SFDC-CLIENT-ID",h="SFDC-LOGIN-HOST-URL",l="SFDC-SESSION-TOKEN",
j,m,o,k,p="standalone"in navigator&&navigator.standalone;return{isActive:function(){return m==true},isEulaAccepted:function(){return k==true},updateEula:function(a){k=a},getClientId:function(){return m?StorageManager.getLocalValue(g):null},getUsername:function(){return m?o:null},getApiClient:function(){return m?j:null},getLoginHostType:function(){return StorageManager.getLocalValue("SFDC-LOGIN-HOST")},getLoginHostUrl:d,setLoginHostType:function(a){a!=ManageUserSession.getLoginHostType()&&StorageManager.setLocalValue("SFDC-LOGIN-HOST",
a)},setLoginHostUrl:function(a){a=a.toLowerCase();if(d()!=a){var b="host_custom";switch(a){case "login.salesforce.com":b="host_production";break;case "test.salesforce.com":b="host_sandbox"}StorageManager.setLocalValue(h,a);ManageUserSession.setLoginHostType(b)}},initialize:function(a,d){j||(j=new sforce.Client);var k=StorageManager.getSessionValue(l),g=StorageManager.getLocalValue(f);if(m)a();else if(k&&k.length>0)b(k,a);else if(g&&g.length>0)c(a);else if(window.location.search.indexOf("code=")>0){k=
/code=([^&]*)&?/.exec(window.location.search)[1];e(k,function(){history.pushState(null,null,window.location.pathname);a()})}else{typeof d=="boolean"&&d&&StorageManager.clearAll();window.location=j.getAuthorizeUrl(ManageUserSession.getLoginHostUrl())}},invalidate:function(a,b){a&&m&&j.revokeSession(null,null,null,null,b);StorageManager.clearSessionValue(l);j=m=o=void 0},kill:function(a){var b=StorageManager.getLocalValue(g),c=StorageManager.getLocalValue(f);j.revokeSession(b,c,null,null,function(){ManageUserSession.invalidate();
...
a.passcodeElem.unbindOrientationChange(c);a=void 0});if(/ipad/gi.test(navigator.platform)||$j(window).width()>$j("#passcode table").outerWidth()+50){b=void 0;a.passcodeElem.addClass("passcodeTransition").css(vendor+"TransitionProperty","opacity").css("opacity","0")}else{b=$j(window).height()+"px";a.passcodeElem.addClass("passcodeTransition").css(vendor+"TransitionProperty","-"+vendor.toLowerCase()+"-transform");a.passcodeElem.css(vendor+"Transform","translateY("+b+")")}},g=function(){if(b===void 0){a.passcodeElem.find("table td.passcode_button").each(function(){var b=
$j(this),c=b.text();b.touch(function(d){d.preventDefault();d.stopPropagation();if(a.elemNum<4){$j(b).addClass("pressed");setTimeout(function(){$j(b).removeClass("pressed")},100);o(c);a.elemNum==4&&a.callback&&a.callback(a.passcode)}})});b=true}},h=function(b){a={passcodeElem:$j("#passcode"),inputElems:$j("#passcode #passcode_input input"),elemNum:0,passcode:"",callback:b};g();m(a.title);l()},l=function(){m("Enter Passcode");$j("#passcode #passcode_message").empty().hide();$j("#passcode #passcode_header").removeClass("passcode_error");
j()},j=function(){a.elemNum=0;a.passcode="";a.inputElems.val("")},m=function(a){$j("#passcode #passcode_title").html(a)},o=function(b){if(b=="Logout"||b=="Cancel")a.callback(-1);else if(b=="Delete"){if(a.elemNum>0){a.inputElems[--a.elemNum].value="";a.passcode=a.passcode.substr(0,a.elemNum)}}else if(parseInt(b)>=0){a.inputElems[a.elemNum++].value=".";a.passcode=a.passcode+b}};return{checkPasscode:function(b,c){h(function(a){var d=function(a,b){if(a){f();typeof c=="function"&&c()}else{j();if(b){m("Wrong Passcode");
$j("#passcode #passcode_message").html(b).show();$j("#passcode #passcode_header").addClass("passcode_error")}}};b&&b(a,d)});a.passcodeElem.find("#cancel").text("Logout");e()},setupPasscode:function(b){var c;h(function(d){if(d==-1){f();typeof b=="function"&&b(-1)}else if(c)if(c==d)typeof b=="function"&&b(a.passcode,f);else{c=void 0;alert("Passcode didn't match. Please try again.");l()}else{c=d;m("Re-enter the passcode");j()}});m("Setup Passcode");a.passcodeElem.find("#cancel").text("Cancel");e()}}}();
