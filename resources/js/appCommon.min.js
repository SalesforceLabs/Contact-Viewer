...
a+" span").text());b.find("#hosts table td").css("color","black");b.find("#hosts #"+a).css("color","#395483").append(b.find("#hosts #check"))}b.find("#main #connection #host_url input").val(ManageUserSession.getLoginHostUrl())},k=function(c){b.find("#header #left").show().touch(function(){b.find("#header #left").hide().unbind();if(useAnimations)c.changePage(b.find("#main"),true,function(){c.css("visibility","hidden")});else{c.hide();b.find("#main").show().css("visibility","")}b.find("#header #title").text("Settings");
ManageUserSession.isActive()&&b.find("#header #done").show();a("#main")})},m=function(c,d){ManageUserSession.getApiClient().getContactAppEula(function(a){a=$j(a);a.find("head, script").remove();b.find("#eula .content").empty().append(a);if(c){a=b.find("#eula_buttons").show();b.find("#eula").css("margin-bottom",a.outerHeight()+"px")}else b.find("#eula").css("margin-bottom","0")},errorCallback,function(b,c){a("#eula");typeof d=="function"&&d(c)})},o=function(c,d,e,f){var n=$j(this),j;g&&setTimeout(function(){b.find("#main #connection #host_url input").blur()},
10);n.addClass("cellselected");b.find("#header #done").hide();j=function(){n.removeClass("cellselected");b.find("#main").css("visibility","hidden");typeof f=="function"&&f()};if(useAnimations)b.find("#main").changePage(b.find(c),false,j);else{b.find("#main").hide();b.find(c).show().css("visibility","");j()}b.find("#header #title").text(d);a(c);e&&k(b.find(c))},j=function(a,b,c){o(a,b,true,c)},p=function(){b.find("#main #connection #hostType").click(function(){j("#hosts","Login Host")});b.find("#hosts table td").touch(function(){var a=
$j(this);a.addClass("cellselected");ManageUserSession.setLoginHostType(this.id);l();setTimeout(function(){a.removeClass("cellselected")},100)});b.find("#main #connection #host_url input").focus(function(){g=true;$j(this).css("text-align","left")}).blur(function(){g=false;$j(this).val(ManageUserSession.getLoginHostUrl());$j(this).css("text-align","right");setTimeout(function(){$j(document.body).scrollTop(0)},10)});b.find("#main #connection #host_url form").submit(function(){var a=b.find("#main #connection #host_url input"),
c=b.find("#main #connection #host_url input").val().trim();(c=c.length==0||c.endsWith(".salesforce.com"))||alert('Custom host url must end with ".salesforce.com".');if(c){ManageUserSession.setLoginHostUrl(a.val());l();a.blur()}return false});b.find("#main #help #help_about").click(function(){j("#about","About")});b.find("#main #help #help_faq").click(function(){j("#faq","FAQ",function(){})});b.find("#main #help #help_eula").click(function(){j("#eula","Contact Viewer EULA",m)});b.find("#loginbtn").unbind().click(function(){prepareSession()}).touch(function(a){a.stopPropagation()});
b.find("#logoutbtn").unbind().click(function(){confirm("Logout user "+ManageUserSession.getUsername()+"?")&&logout(false)}).touch(function(a){a.stopPropagation()})},n=function(){b.positionCenterOf(window)};return{show:function(){var d,e;if(!c){c=true;b=$j("#app_settings");p()}l();b.find("#header #title").text("Settings");b.find(".page").css("visibility","hidden");b.find("#main").css("visibility","visible");b.width()<500&&b.find("#header #left>div").text("Back");if(ManageUserSession.isActive()){b.find("#header #done").show();
b.find("#loginbtn").hide();b.find("#logoutbtn").show()}else{b.find("#loginbtn").show();b.find("#logoutbtn").hide()}f=$j("#loggedin").addOverlay();b.find("#username").setText(ManageUserSession.getUsername()||"None");b.find("#header #done").unbind("click").click(function(){SettingsManager.hide();$j(this).unbind("click");if(!ManageUserSession.isActive())window.location=getBaseUrl()});ManageUserSession.isActive()||b.find("#header #done").hide();d=window.innerHeight;e=(window.innerHeight-b.height())/2;
b.hide().css("zIndex",$j.topZIndex(f.elem)+10).css({left:(window.innerWidth-b.width())/2});useAnimations?b.css({top:0}).show().slideIn("Y",d,e):b.css({top:e}).show();a("#main");b.orientationChange(n)},hide:function(a){var c=window.innerHeight,d=function(){b.hide();f.hide();typeof a=="function"&&a()};useAnimations?b.slideOut("Y",c,d):d();h();b.unbindOrientationChange(n)},showEula:function(a,c){SettingsManager.show();o("#eula","End User License Agreement",false,function(){m(true);var d=b.find("#eula_buttons").show();
d.find("#accept").unbind().click(a);d.find("#decline").unbind().click(c)})},hideEula:function(){SettingsManager.hide(function(){if(useAnimations)b.find("#eula").changePage(b.find("#main"),true);else{b.find("#eula").hide();b.find("#main").show().css("visibility","")}b.find("#eula").css("visibility","hidden");b.find("#eula_buttons").hide();b.find("#header #title").text("Settings")})}}}(),FeedPhotoRenderer=function(){this.userIds=[]};
...
var ManageUserSession=function(){function a(a){m=true;o=a.username;j=a.eula;k.setSessionHeader(a.sessionToken);StorageManager.setSessionValue(l,a.sessionToken)}function b(b,c){k.setSessionHeader(b);var d=function(){k.setSessionHeader(null);StorageManager.clearSessionValue(l);ManageUserSession.initialize(c)};k.prepareSession(function(b){if(b.success){a(b);typeof c=="function"&&c()}else d()},d)}function c(b){PasscodeManager.checkPasscode(function(c,d){if(c==-1)confirm("Logout user?")?logout(true):typeof d==
"function"&&d(false);else{var e=$j(document).showActivityInd(loadingImg,"Authenticating...",false),j=StorageManager.getLocalValue(g),h=StorageManager.getLocalValue(f);k.refreshAccessToken(c,j,h,function(c){if(c.success){a(c);typeof d=="function"&&d(true);typeof b=="function"&&b()}else{c.errorCode=="INVALID_TOKEN"&&StorageManager.clearAll();typeof d=="function"&&d(c.success,c.error)}},function(a,b){typeof d=="function"&&d(false,b=="error"?"Server Unavailable. Check network connection or try again later.":
"Request timed out. Please try again.")},e.hide)}})}function d(){var a;switch(ManageUserSession.getLoginHostType()){case "host_production":a="login.salesforce.com";break;case "host_sandbox":a="test.salesforce.com";break;case "host_custom":a=StorageManager.getLocalValue(h)}if(!a||a.length==0)a="login.salesforce.com";return a}function e(b,c){var d=function(d,e){var d=d==-1?void 0:d,j=$j(document).showActivityInd(loadingImg,d?"Setting Passcode...":"Authenticating...",false);k.obtainAccessToken(ManageUserSession.getLoginHostUrl(),
b,d,function(b){j.hide();if(b.success){a(b);var n=b.clientId,b=d?b.refreshToken:null;n&&n.length>0&&StorageManager.setLocalValue(g,n);b&&b.length>0&&StorageManager.setLocalValue(f,b);typeof c=="function"&&c()}else alert(b.error);typeof e=="function"&&e()},function(){if(confirm("Failed to obtain access. Try again!"))window.location=k.getAuthorizeUrl(ManageUserSession.getLoginHostUrl())})};p?PasscodeManager.setupPasscode(d):d()}var f="SFDC-OAUTH-TOKEN",g="SFDC-CLIENT-ID",h="SFDC-LOGIN-HOST-URL",l="SFDC-SESSION-TOKEN",
k,m,o,j,p="standalone"in navigator&&navigator.standalone;return{isActive:function(){return m==true},isEulaAccepted:function(){return j==true},updateEula:function(a){j=a},getClientId:function(){return m?StorageManager.getLocalValue(g):null},getUsername:function(){return m?o:null},getApiClient:function(){return m?k:null},getLoginHostType:function(){return StorageManager.getLocalValue("SFDC-LOGIN-HOST")},getLoginHostUrl:d,setLoginHostType:function(a){a!=ManageUserSession.getLoginHostType()&&StorageManager.setLocalValue("SFDC-LOGIN-HOST",
a)},setLoginHostUrl:function(a){a=a.toLowerCase();if(d()!=a){var b="host_custom";switch(a){case "login.salesforce.com":b="host_production";break;case "test.salesforce.com":b="host_sandbox"}StorageManager.setLocalValue(h,a);ManageUserSession.setLoginHostType(b)}},initialize:function(a,d){k||(k=new sforce.Client);var j=StorageManager.getSessionValue(l),g=StorageManager.getLocalValue(f);if(m)a();else if(j&&j.length>0)b(j,a);else if(g&&g.length>0)c(a);else if(window.location.search.indexOf("code=")>0){j=
/code=([^&]*)&?/.exec(window.location.search)[1];e(j,function(){history.pushState(null,null,window.location.pathname);a()})}else{typeof d=="boolean"&&d&&StorageManager.clearAll();window.location=k.getAuthorizeUrl(ManageUserSession.getLoginHostUrl())}},invalidate:function(a,b){a&&m&&k.revokeSession(null,null,null,null,b);StorageManager.clearSessionValue(l);k=m=o=void 0},kill:function(a){var b=StorageManager.getLocalValue(g),c=StorageManager.getLocalValue(f);k.revokeSession(b,c,null,null,function(){ManageUserSession.invalidate();
