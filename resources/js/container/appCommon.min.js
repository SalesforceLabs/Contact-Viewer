...
function(){typeof b.contactListPullDownCB=="function"&&b.contactListPullDownCB()});$j(window).orientationChange(b._initiateContactListScroller)}else b.listpagescroll&&b.listpagescroll.refresh()},displayList:function(a,b){var d;this.listFetchStartTime=Date.now();d=a?this.view.find("#header #listselect button#"+a):this.view.find("#header #listselect button#recent");this.view.find("#header #titlebar #title").text(d.text());this.selectedListId=d[0].id;if(typeof this.options.onListSelect=="function")this.options.onListSelect(this.selectedListId,
b)},updateList:function(a){var b=this,d=b.view.find("#listscroller #scroller #contactlist"),c=b.view.find("#listscroller #scroller #resultCount");d.empty();c.empty();!a||a.length==0?c.text("No "+contactPluralLabel+" Found"):c.text("Found "+a.length+" "+contactPluralLabel);a.sort(function(a,b){var d=a.LastName.toLowerCase(),c=b.LastName.toLowerCase();return d==c?0:d>c?1:-1});var e=[];$j.each(a,function(){var a=htmlEncode(this.Id);if(e.length==0||this.LastName!=null&&e.last()!=this.LastName[0].toUpperCase()){e.push(this.LastName[0].toUpperCase());
$j("<li></li>").addClass("listseparater").text(e.last()).appendTo(d)}var c=$j("<li></li>").attr("id","contact_"+a);a==b.selectedContactId&&c.addClass("cellselected");c.text((this.FirstName||"")+" ").append($j("<strong></strong>").text(this.LastName||"")).appendTo(d)});b.mode=="search"?LocalyticsManager.tagSearch(b.searchTextLength,a.length,Date.now()-b.listFetchStartTime):LocalyticsManager.tagListView(this.selectedListId,a.length,Date.now()-b.listFetchStartTime)},resetSelectedContact:function(){this.selectedContactId=
