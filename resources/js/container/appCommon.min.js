...
var SettingsManager=function(){function a(d){if(d&&e!=d){c&&h();c=createScroller(b.find(".settings_body "+d));e=d;$j(window).orientationChange(function(){a()})}else c&&c.refresh();b.find(".settings_body "+e+" a").unbind().touch(function(a){a.stopPropagation()})}var b,d,c,e,f,g=false,h=function(){if(c){c.destroy();e=c=void 0}},j=function(){var a=ManageUserSession.getLoginHostType();if(!a){a=b.find("#hosts table td")[0].id;ManageUserSession.setLoginHostType(a)}if(a){b.find("#connection #hostType #name").text(b.find("#hosts #"+
a+" span").text());b.find("#hosts table td").css("color","black");b.find("#hosts #"+a).css("color","#395483").append(b.find("#hosts #check"))}b.find("#main #connection #host_url input").val(ManageUserSession.getLoginHostUrl())},k=function(d,c){ManageUserSession.getApiClient().getContactAppEula(function(a){a=$j(a);a.find("head, script").remove();b.find("#eula .content").empty().append(a);if(d){a=b.find("#eula_buttons").show();b.find("#eula").css("margin-bottom",a.outerHeight()+"px")}else b.find("#eula").css("margin-bottom",
"0")},errorCallback,function(b,d){a("#eula");typeof c=="function"&&c(d)})},l=function(d,c,e,f){var h=$j(this),j;g&&setTimeout(function(){b.find("#main #connection #host_url input").blur()},10);h.addClass("cellselected");b.find("#header #done").hide();j=function(){h.removeClass("cellselected");b.find("#main").css("visibility","hidden");typeof f=="function"&&f()};if(useAnimations)b.find("#main").changePage(b.find(d),false,j);else{b.find("#main").hide();b.find(d).show().css("visibility","");j()}b.find("#header #title").text(c);
a(d);if(e){var k=b.find(d);b.find("#header #left").show().touch(function(){b.find("#header #left").hide().unbind();if(useAnimations)k.changePage(b.find("#main"),true,function(){k.css("visibility","hidden")});else{k.hide();b.find("#main").show().css("visibility","")}b.find("#header #title").text("Settings");ManageUserSession.isActive()&&b.find("#header #done").show();a("#main")})}},m=function(){b.positionCenterOf(window)};return{show:function(){var c,e;if(!d){d=true;b=$j("#app_settings");b.find("#main #connection #hostType").click(function(){l("#hosts",
"Login Host",true,void 0)});b.find("#hosts table td").touch(function(){var a=$j(this);a.addClass("cellselected");ManageUserSession.setLoginHostType(this.id);j();setTimeout(function(){a.removeClass("cellselected")},100)});b.find("#main #connection #host_url input").focus(function(){g=true;$j(this).css("text-align","left")}).blur(function(){g=false;$j(this).val(ManageUserSession.getLoginHostUrl());$j(this).css("text-align","right");setTimeout(function(){$j(document.body).scrollTop(0)},10)});b.find("#main #connection #host_url form").submit(function(){var a=
b.find("#main #connection #host_url input"),d=b.find("#main #connection #host_url input").val().trim();(d=d.length==0||d.endsWith(".salesforce.com"))||alert('Custom host url must end with ".salesforce.com".');if(d){ManageUserSession.setLoginHostUrl(a.val());j();a.blur()}return false});b.find("#main #help #help_about").click(function(){l("#about","About",true,void 0)});b.find("#main #help #help_faq").click(function(){l("#faq","FAQ",true,function(){})});b.find("#main #help #help_eula").click(function(){l("#eula",
"Contact Viewer EULA",true,k)});b.find("#loginbtn").unbind().click(function(){prepareSession()}).touch(function(a){a.stopPropagation()});b.find("#logoutbtn").unbind().click(function(){confirm("Logout user "+ManageUserSession.getUsername()+"?")&&logout(false)}).touch(function(a){a.stopPropagation()})}j();b.find("#header #title").text("Settings");b.find(".page").css("visibility","hidden");b.find("#main").css("visibility","visible");b.width()<500&&b.find("#header #left>div").text("Back");if(ManageUserSession.isActive()){b.find("#header #done").show();
b.find("#loginbtn").hide();b.find("#logoutbtn").show()}else{b.find("#loginbtn").show();b.find("#logoutbtn").hide()}f=$j("#loggedin").addOverlay();b.find("#username").setText(ManageUserSession.getUsername()||"None");b.find("#header #done").unbind("click").click(function(){SettingsManager.hide();$j(this).unbind("click");if(!ManageUserSession.isActive())window.location=getBaseUrl()});ManageUserSession.isActive()||b.find("#header #done").hide();c=window.innerHeight;e=(window.innerHeight-b.height())/2;
b.hide().css("zIndex",$j.topZIndex(f.elem)+10).css({left:(window.innerWidth-b.width())/2});useAnimations?b.css({top:0}).show().slideIn("Y",c,e):b.css({top:e}).show();a("#main");b.orientationChange(m)},hide:function(a){var d=window.innerHeight,c=function(){b.hide();f.hide();typeof a=="function"&&a()};useAnimations?b.slideOut("Y",d,c):c();h();b.unbindOrientationChange(m)},showEula:function(a,d){SettingsManager.show();l("#eula","End User License Agreement",false,function(){k(true);var c=b.find("#eula_buttons").show();
c.find("#accept").unbind().click(a);c.find("#decline").unbind().click(d)})},hideEula:function(){SettingsManager.hide(function(){if(useAnimations)b.find("#eula").changePage(b.find("#main"),true);else{b.find("#eula").hide();b.find("#main").show().css("visibility","")}b.find("#eula").css("visibility","hidden");b.find("#eula_buttons").hide();b.find("#header #title").text("Settings")})}}}(),FeedPhotoRenderer=function(){this.userIds=[]};
...
void 0===sforce.ListView&&(sforce.ListView=function(a){this.options={onListSelect:null,onSearch:null,onItemSelect:null,listTypes:{owner:"My Records",follow:"Records I Follow",recent:"Recently Viewed"}};for(i in this.options)this.options[i]=a[i];this.selectedContactId=a.selectedContactId;this.view=$j("#listpage");this._init()},sforce.ListView.prototype={_init:function(){var a=this,b=function(b){var c=a.view.find("#header #listselect");c.bind("webkitTransitionEnd",function(){b&&b.hide();a.view.find("#header #searchbar").show();
c.unbind("webkitTransitionEnd").hide();c.css("zIndex","");c.prev().css("zIndex","");a.showingListSelector=false});c.css("-webkit-transform","translateY(-"+c.height()+"px)")};a._addSearchListeners();a.view.find("#header #titlebar").unbind().click(function(){if(!a.showingListSelector){a.showingListSelector=true;var d=a.view.find("#header #listselect"),c=a.view.find("#listscroller").addOverlay(),e=$j.topZIndex(c.elem);d.prev().css("zIndex",e+2);d.css("zIndex",e+1);d.css(vendor+"TransitionProperty","").css("-webkit-transform",
"translateY(-"+d.height()+"px)");d.css(vendor+"TransitionProperty","-webkit-transform").css("visibility","hidden").show("fast",function(){a.view.find("#header #searchbar").hide();d.css("visibility","").css("-webkit-transform","translateY(0px)");d.find("button").windowTouch(function(d){d.preventDefault();d=d.target;a.view.find("#header #searchbar>form>input[type=search]").val("");a.view.find("#header #searchbar>form>button").hide();b(c);a._displayList(d.id);return true},function(a){var d=a.target;
if(d.nodeType==3)d=d.parentNode;var e=$j("#listpage");($j(d).is(e)||e.has(d).length>0)&&a.preventDefault();b(c);return true},true)})}});a.view.find("#listscroller #scroller #contactlist").unbind().click(function(b){var c=b.target;if(!a.loadingItem){c=$j(c).closest("li",a.view);b.preventDefault();c.length&&(!c.hasClass("listseparater")&&typeof a.options.onItemSelect=="function")&&a.selectContact(c[0].id.substring(8))}})},_addSearchListeners:function(){var a=this,b=function(){var a=$j(this);a.val().length>
