...
function sessionCallback(){$j("#loggedin").css("display","block");addClickListeners();var a,b,c=$j("#loggedin").showActivityInd("Loading..."),d=StorageManager.getLocalValue(last_visited_loc_storage_key);d&&2==d.split("/").length&&(d=d.split("/"),b=d[0],a=function(a){a&&(c.hide(),showContact(b))});fetchContactDescribe(a);sobjectModel={setRecords:function(a){this.records=a;this.recordIds=[];for(i in a)this.recordIds.push(a[i].Id)}};splitView=new sforce.SplitView;listView=new sforce.ListView({selectedContactId:b,
onListSelect:getContacts,onSearch:searchContacts,onItemSelect:showContact});splitView.addOrientationChangeCallback(function(){listView.refreshScroller()});listView.displayList("recent",c.hide);"function"==typeof window.addToHomeLaunch&&window.addToHomeLaunch()}function switchToDetail(a){useAnimations?$j("#listpage").changePage("#detailpage",!1,a):($j("#listpage").hide(),$j("#detailpage").show().css("visibility",""),"function"==typeof a&&a())}
function addClickListeners(){$j("#listpage #header #gear").off().enableTap().click(function(a){a.preventDefault();SettingsManager.show()})}
function showContact(a,b){$j("#detailpage #detail").hide();$j("#detailpage .header>span").empty();switchToDetail();var c=$j("#loggedin").showActivityInd("Loading...");switchDetailSection("info",[a],function(a){a&&$j("#detailpage #detail").show();$j("#detailpage .header #left").off().enableTap().click(function(){var a=function(){$j("#detailpage").css("visibility","hidden");listView.resetSelectedContact()};$j("#detailpage .header #left").off();useAnimations?$j("#detailpage").changePage("#listpage",
!0,a):($j("#detailpage").hide(),$j("#listpage").show().css("visibility",""),a());updateLastVisitLoc()});c.hide();"function"==typeof b&&b()});updateLastVisitLoc(a+"/info");"function"==typeof showContactPicture&&showContactPicture(a)}
...
displayContactDetails(a),c=!0):alert(contactLabel+" not found. Please refresh the list and try again.")},errorCallback,function(){"function"==typeof b&&(setTimeout(function(){initiateInfoScroller();infopagescroll.scrollTo(0,0,0)},10),b(c))})}
function switchDetailSection(a,b,c){var d=Date.now();$j("#detailpage #contactInfo").show();$j("#detailpage #detail>span>div").hide();var e=function(b){LocalyticsManager.tagDetailView(a,Date.now()-d);"function"==typeof c&&c(b)};"info"==a?renderContactInfo(b[0],function(a){a&&$j("#detailpage #detail #infoscroller").show();e(a)}):e();updateLastVisitLoc(b[0]+"/"+a)};