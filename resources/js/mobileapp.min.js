...
function sessionCallback(){$j("#loggedin").css("display","block");addClickListeners();var a,c,b=$j("#loggedin").showActivityInd(loadingImg,"Loading..."),d=StorageManager.getLocalValue(last_visited_loc_storage_key);d&&2==d.split("/").length&&(d=d.split("/"),c=d[0],a=function(a){a&&(b.hide(),showContact(c))});fetchContactDescribe(a);sobjectModel={setRecords:function(a){this.records=a;this.recordIds=[];for(i in a)this.recordIds.push(a[i].Id)}};splitView=new sforce.SplitView;listView=new sforce.ListView({selectedContactId:c,
onListSelect:getContacts,onSearch:searchContacts,onItemSelect:showContact});splitView.addOrientationChangeCallback(function(){listView.refreshScroller()});getContacts("recent",b.hide);"function"==typeof window.addToHomeLaunch&&window.addToHomeLaunch()}function switchToDetail(a){$j("#listpage").changePage("#detailpage",!1,function(){typeof a=="function"&&a()})}function addClickListeners(){$j("#listpage #header #gear").unbind().touch(function(a){a.preventDefault();SettingsManager.show()})}
function showContact(a,c){$j("#detailpage #detail").hide();$j("#detailpage .header>span").empty();switchToDetail();var b=$j("#loggedin").showActivityInd(loadingImg,"Loading...");switchDetailSection("info",[a],function(a){a&&$j("#detailpage #detail").show();$j("#detailpage .header #left").unbind().click(function(){$j("#detailpage .header #left").unbind();$j("#detailpage").changePage("#listpage",!0,function(){$j("#detailpage").css("visibility","hidden");listView.resetSelectedContact()});updateLastVisitLoc()});
b.hide();"function"==typeof c&&c()});updateLastVisitLoc(a+"/info")}
function displayContactSummary(a){$j("#Id").val(a.Id);getFieldDescribe();var c=$j("#detailpage #detail");c.find("#summary #photo_div>span").html(formatStr(a.Name,25)).append("<br/>").append(formatStr(a.Title,25));if(a.Phone){var b=cleanupPhone(a.Phone);c.find("#call_contact #skype").attr("href","skype:"+b+"?call").show();/ipad|iphone/gi.test(navigator.platform)?c.find("#call_contact #facetime").attr("href","facetime://"+b).show():c.find("#call_contact #facetime").hide()}else c.find("#call_contact #skype").attr("href",
"#").hide(),c.find("#call_contact #facetime").attr("href","#").hide();a.Email?c.find("#call_contact #email").attr("href","mailto:"+a.Email).show():c.find("#call_contact #email").attr("href","#").hide()}
function displayContactDetails(a){var c=getFieldDescribe(),b=$j("#detailpage #detail #info");c.AccountId?(b.find("#Account").show(),b.find("#Account .fieldLbl").text(c.AccountId.relationshipLabel),b.find("#Account .fieldVal").html(a.Account?formatStr(a.Account.Name,50):"&nbsp;")):b.find("#Account").hide();c.Department?(b.find("#Department").show(),b.find("#Department .fieldLbl").text(c.Department.label),b.find("#Department .fieldVal").html(formatStr(a.Department,50)||"&nbsp;")):b.find("#Department").hide();
...
void 0!=c.MailingStreet?(a=formatAddress(a),b.find("#Address").show(),b.find("#Address .fieldLbl").text("Mailing Address"),0<a.length?(a='<a href="http://maps.google.com/maps?daddr='+a.replace(/\n/g,", ")+'">'+a.replace(/\n/g,"<br/>")+"</a>",b.find("#Address .fieldVal").html(a)):b.find("#Address .fieldVal").empty()):b.find("#Address").hide()}
