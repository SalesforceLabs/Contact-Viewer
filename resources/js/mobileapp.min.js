...
function sessionCallback(){var a,c,b,d=StorageManager.getLocalValue(last_visited_loc_storage_key);$j("#loggedin").css("display","block");b=$j("#loggedin").showActivityInd("Loading...");addClickListeners();d&&2==d.split("/").length&&(d=d.split("/"),c=d[0],a=function(a){a&&(b.hide(),showContact(c,LocalyticsManager.logAppReady))});fetchContactDescribe(a);sobjectModel={setRecords:function(a){this.records=a;this.recordIds=[];for(i in a)this.recordIds.push(a[i].Id)}};splitView=new sforce.SplitView;listView=
new sforce.ListView({selectedContactId:c,onListSelect:getContacts,onSearch:searchContacts,onItemSelect:showContact});splitView.addOrientationChangeCallback(function(a){listView.refreshScroller();LocalyticsManager.tagScreenOrientation(a)});listView.displayList("recent",b.hide);"function"==typeof window.addToHomeLaunch&&window.addToHomeLaunch()}
...
function showContact(a,c){var b,d;$j("#detailpage #detail").hide();$j("#detailpage .header>span").empty();switchToDetail();b=$j("#loggedin").showActivityInd("Loading...");d=switchDetailSection("info",[a],function(a){a&&$j("#detailpage #detail").show();b.hide();"function"==typeof c&&c()});$j("#detailpage .header #left").off().enableTap().click(function(){var a=function(){$j("#detailpage").css("visibility","hidden");listView.resetSelectedContact()};d.abort();$j("#detailpage .header #left").off();useAnimations?
...
displayContactSummary(a),displayContactDetails(a),b=!0):alert(contactLabel+" not found. Please refresh the list and try again.")},errorCallback,function(){"function"==typeof c&&(setTimeout(function(){initiateInfoScroller();infopagescroll.scrollTo(0,0,0)},10),c(b))})}
function switchDetailSection(a,c,b){var d=Date.now();$j("#detailpage #contactInfo").show();$j("#detailpage #detail>span>div").hide();var e=function(c){LocalyticsManager.tagDetailView(a,Date.now()-d);"function"==typeof b&&b(c)};if("info"==a)return LocalyticsManager.tagScreen("Detail"),renderContactInfo(c[0],function(a){a&&$j("#detailpage #detail #infoscroller").show();e(a)});e();updateLastVisitLoc(c[0]+"/"+a)};