...
function renderContactInfo(a,c){var b=!1,d="Id Name Account.Id Account.Name Department Title Phone MobilePhone Email MailingStreet MailingCity MailingState MailingCountry MailingPostalCode ReportsTo.Name".split(" ");hasRecordTypes&&d.push("RecordTypeId");var f=getFieldDescribe(),d=d.filter(function(b){b=b.split(".");return f[b[0]]});$j("#rightsection #detailpage #detail #info");ManageUserSession.getApiClient().retrieveContactViaApex(a,d,function(a){a&&"Id"in a?($j("#detailpage .header>span").text(a.Name),
...
function renderContactLayout(a,c){var b=$j("#rightsection #detailpage #detail #info #detailsLayout");b.find("#layout").empty();b.find("#viewMoreBtn").find("span").text("Load All Details");b.find("#viewMoreBtn").find("img").hide();b.find("#viewMoreBtn").show().off().click(function(){$j(this).off().find("span").text("Loading Details...");$j(this).find("img").show();ManageUserSession.getApiClient().getContactDetailsViaApex(a,c,function(a){a=$j(a);a.find("head, script").remove();formatDetailContent(b.find("#layout").empty().append(a));
...
function codeAddressOnMap(a){var c=encodeURI(a);$j("#map_section #map_div #google_map_canvas").empty().append('<img src="https://maps.googleapis.com/maps/api/staticmap?zoom=14&size=640x180&format=jpeg&sensor=false&markers=color:red%7C'+c+'"/>');$j("#map_section #map_div #openMaps").off().click(function(){var a=("undefined"!=typeof PhoneGap&&PhoneGap?"maps:q=":"https://maps.google.com/maps?q=")+c;/iphone|ipod|ipad/gi.test(navigator.platform)?window.location=a:window.open(a)})};